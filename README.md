...
> **Status:** DEMO 版完成 — タイトルからゲームオーバー・ランキングまで一通りプレイ可能。今後はコンテンツ拡張とバランス調整
...
- `lib/audioPaths.ts` / `lib/effectPaths.ts` — BGM・SE・戦闘エフェクトのパスを定義
...
- [x] スコア永続化とランキング Top 100 表示（PostgreSQL + JPA）
- [x] フロントエンド全画面と API 連携（タイトル → 探索 → 各イベント → ゲームオーバー → ランキング）
- [x] 全画面のレイアウト
- [x] 戦闘アニメーション（ダメージ表示・エフェクト・撃破演出）
- [x] BGM / SE と音量設定
- [x] レスポンシブデザイン（スマホ用のレイアウト）

### 今後の拡張予定

- [ ] バランス調整（敵の強さ・報酬・スコア計算など）
- [ ] コンテンツ追加（敵・装備・カード・アイテムのバリエーション）
- [ ] ゲーム体験の深化（イベント追加、リプレイ性の向上）
---
...
| API | 全エンドポイントが `X-Session-Id` ヘッダーで対象セッションを特定 |
...
│       ├── service/
│       │   ├── GameSessionManager.java  # sessionId ごとの状態管理
│       │   └── gamestate/
│       │       └── session/          # GameSession（1 プレイ分の状態）
│       ├── domain/                   # 値オブジェクト
│       ├── dto/                      # リクエスト / レスポンス
│       ├── entity/                   # JPA エンティティ
│       ├── repository/               # DB リポジトリ
...
│       │   ├── atoms/                # 最小 UI 部品
│       │   ├── molecules/            # 複合 UI 部品
│       │   └── providers/            # AudioProvider など
│       ├── hooks/                    # セッション・画面ガード
│       ├── lib/                      # API クライアント・セッション・音声パス
│       └── type/                     # 型定義
...
| POST | `/score/register` | スコア登録（サーバー側で計算） |
| GET | `/ranking` | ランキング取得 |
...
1. **バランス調整** — 敵の強さ・報酬・スコア計算のチューニング
2. **コンテンツ拡張** — 敵・装備・カード・アイテムの追加
3. **ゲーム体験の深化** — 新イベント・リプレイ性の向上
