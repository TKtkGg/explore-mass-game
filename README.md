...
> **Status:** 開発中（WIP） — 全画面のレイアウトは概ね完成。バトル画面のアニメーション実装を進行中
...
| レイヤ | 例 |
|--------|-----|
| atoms | `MainButton`, `Title`, `ErrorAlert`, `HpBar`, `Input`, `TwoRowTitle` |
| molecules | `RouteAllowButton`, `CardButton`, `TreasureButton`, `BattleEnemyDisplay`, `BattleCommandBox`, `BattleMessageBox`, `BattleResultModal`, `Tooltip` |
...
- [x] フロントエンド全画面と API 連携
- [x] 全画面のレイアウト（探索・戦闘・ショップ・ステータス・装備・アイテム・カード一覧・宝箱・ゲームオーバー・ランキング・タイトル）
- [x] 戦闘画面の UI コンポーネント分割（敵表示・コマンドボックス・メッセージ・結果モーダル）

### 開発中

- [ ] バトル画面のアニメーション（攻撃・被ダメージ・勝敗演出など）
...
1. **バトル画面のアニメーション** — 攻撃・被ダメージ・勝敗など、戦闘の手応えが伝わる演出の追加
